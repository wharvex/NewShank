...
namespace Shank {
    public class Lexer {
        private enum modeType {
            Start, Number, Name, Comment, SingleQuote, DoubleQuote
        }

        private modeType mode = modeType.Start;
        private Dictionary<string, Token.TokenType> ReservedWords = new(){
        ...
        private void ModeChange(List<Token> retVal, StringBuilder currentBuffer, int lineNumber)
        {
            switch (mode)
            {
                ...
                case modeType.Name:
                    ...
                    retVal.Add(ReservedWords.ContainsKey(val)
                        ? new Token(ReservedWords[val],lineNumber)
                        : new Token(Token.TokenType.Identifier, lineNumber, val));
                    currentBuffer.Clear();
                    break;
                case modeType.SingleQuote:
                    retVal.Add(new Token(Token.TokenType.CharContents,lineNumber,  currentBuffer.ToString()));
                    if (currentBuffer.Length > 1)
                        throw new Exception($"Character literals can only be one character long. Found: {currentBuffer}");
                    currentBuffer.Clear();
                    break;
                case modeType.DoubleQuote:
                    retVal.Add(new Token(Token.TokenType.StringContents,lineNumber,  currentBuffer.ToString()));
                    currentBuffer.Clear();
                    break;
                ...
                for (; index < line.Length; index++) // continue on
                {
                    ...
                    else
                    {
                        switch (character)
                        {
                            ...
                            case >= 'A' and <= 'Z' or (>= 'a' and <= 'z') when mode != modeType.Name:
                                if (mode != modeType.Name) ModeChange(retVal, currentBuffer,lineNumber);
                                mode = modeType.Name;
                                currentBuffer.Append(character);
                                break;
                            case >= 'A' and <= 'Z' or (>= 'a' and <= 'z') or (>= '0' and <= '9' or '.')
                                when mode == modeType.Name:
                                currentBuffer.Append(character);
                                break;
                            case (>= '0' and <= '9' or '.') when (mode is modeType.Start or modeType.Number):
                                ...
                                currentBuffer.Append(character);
                                break;
                            case '-' or '+' when mode is modeType.Number or modeType.Name:
                                ModeChange(retVal, currentBuffer,lineNumber);
                                retVal.Add(new Token(character == '+' ? Token.TokenType.Plus : Token.TokenType.Minus,lineNumber));
                                break;
                            case '-' or '+' when mode is modeType.Start:
                                ModeChange(retVal, currentBuffer,lineNumber);
                                if (index < line.Length - 1)
                                {
                                    var next = line[index + 1];
                                    if (next is >= '0' and <= '9')
                                    {
                                        ...
                                        retVal.Add(new Token(character == '+'
                                            ? Token.TokenType.Plus
                                            : Token.TokenType.Minus,lineNumber));
                                }
                                else
                                {
                            ...
                            case ('*' or '/'):
                                ...
                                retVal.Add(new Token(character == '*'
                                    ...
                                    : Token.TokenType.Divide,lineNumber));
                                break;
                            case ')':
                                ModeChange(retVal, currentBuffer,lineNumber);
